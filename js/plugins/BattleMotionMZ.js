 ...
 * @param motionCol
 * @text モーションカラーのループ制御
 * @desc オンにするとモーションカラーでループ制御できるようになる。
 * @type boolean
 * @default true
 *
 * @param blueFps
 * @text 画像の青要素のスピード調整
 * @desc オンにすると画像の青要素でスピードを調整できるようになる。
 * @type boolean
 * @default true
 *
 * @param drawWeapon
 * @text 武器を描画するかどうか。
 * @desc オンにすると武器を描画できるようになる。
 * @type boolean
 * @default false
 *
 * @help
...
var parameters = PluginManager.parameters('BattleMotionMZ');
...
//var _Sprite_Actor_prototype_updateFrame = Sprite_Actor.prototype.updateFrame;
Sprite_Actor.prototype.updateFrame = function() {

//	_Sprite_Actor_prototype_updateFrame.call(this);
	Sprite_Battler.prototype.updateFrame.call(this);

	if(this._mainSprite){
		var bitmap = this._mainSprite.bitmap;
		if (!this._mainSprite.bitmap)  return;
		if (!this._mainSprite.bitmap.isReady())  return;
		...
		this.setFrame(0, 0, cw, ch);	//ようわからん
//		this.setFrame(0, 0, cw/2, ch/2);	//ようわからん
//		this.setFrame(0, 0, this.offsetX, this.offsetY);
	}
};
